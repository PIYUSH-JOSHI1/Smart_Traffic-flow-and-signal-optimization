...
class Vehicle(pygame.sprite.Sprite):
    ...
    def move(self):
     if self.direction == 'up':
        # Vehicles cannot enter from the 'up' direction
        return
     ...
     if self.direction == 'right':
        ...
        if (self.crossed == 0 and self.y + self.currentImage.get_rect().height > stopLines[
            self.direction]):
            self.crossed = 1
            vehicles[self.direction]['crossed'] += 1
        if (self.willTurn == 1):
...
class Main:
    ...
    background = pygame.image.load('oneway.png')
    ...
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                sys.exit()
        screen.fill((0, 0, 0))  # Fill the screen with black color (assuming you want a black background)
        for i in range(0,
